"""
Prompt-injection defenses for untrusted PDF text: strip zero-width
obfuscation characters, detect/defang adversarial command patterns before
they reach any agent prompt, wrap untrusted content in escaped XML tags,
and check LLM output for signs of prompt leakage. Pure stdlib, no external
deps -- ported from the shelved vivek-RAGAS-deepeval branch (see
docs/CONTEXT.md's branch history), where it was the one piece that worked
as-is with no integration gaps.

Wired in at core/parsing/docling_parser.py: every free-text field pulled
out of a PDF (title, abstract, section text, table captions, reference
text) is sanitized once, right after parsing, before any agent ever sees
it -- so downstream code never has to think about this.
"""
import re
import logging
...
# Signatures for typical indirect prompt injection strings. Case-insensitivity
# is applied once at compile time (re.IGNORECASE below) rather than via a
# per-pattern inline (?i) -- joining multiple inline-flagged patterns with
# "|" only works by legacy leniency in current Python and raises a
# DeprecationWarning (a hard error in future versions), since an inline flag
# is only valid at the very start of the whole compiled expression.
ADVERSARIAL_PATTERNS = [
    r"ignore\s+(all\s+)?prior\s+instructions",
    r"ignore\s+(all\s+)?previous\s+instructions",
    r"system\s+override",
    r"attention\s+reviewer",
    r"you\s+must\s+output\s+a\s+perfect\s+score",
    r"override\s+system\s+scoring",
    r"skip\s+(all\s+)?tasks",
    r"bypass\s+evaluation",
    r"format\s+output\s+as\s+follows\s+and\s+ignore\s+constraints"
]

COMPILED_ADVERSARIAL_RE = re.compile("|".join(ADVERSARIAL_PATTERNS), re.IGNORECASE)
